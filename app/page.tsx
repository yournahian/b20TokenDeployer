...
export default function Home() {
  ...
  useEffect(() => {
    const autoConnect = async () => {
      ...
      const previouslyConnected = localStorage.getItem('wallet_connected_previously');
      ...
      try {
        ...
        if (accounts && accounts[0]) {
          ...
          await fetchBalance(address, activeChainId);
          ...
          (window as any).ethereum.on('accountsChanged', (newAccounts: any) => {
            if (newAccounts.length === 0) {
              ...
              fetchBalance(newAccounts[0], chainId);
            }
          });

          (window as any).ethereum.on('chainChanged', (newChainHex: any) => {
            const newChainId = parseInt(newChainHex, 16);
            setChainId(newChainId);
            if (address) {
              fetchBalance(address, newChainId);
            }
          });
        }
      } catch (err) {
  ...
  const fetchBalance = async (address: string, activeChainId: number) => {
    ...
    try {
      ...
      const balBigInt = BigInt(balanceHex);
      const balStr = (Number(balBigInt) / 1e18).toFixed(4);
    ...
    try {
      ...
      try {
        await (window as any).ethereum.request({
          method: 'eth_call',
          params: [callParams, tx.blockNumber ? '0x' + (BigInt(tx.blockNumber) - 1n).toString(16) : 'latest']
        });
      } catch (callErr: any) {
  ...
  const connectWallet = async (forceSelect = true) => {
    if (typeof window === 'undefined' || !(window as any).ethereum) {
      alert('Ethereum wallet is not detected. Please install MetaMask or Coinbase Wallet.');
      return;
    }
    ...
    try {
      if (forceSelect) {
        try {
          // Request permissions to prompt the wallet account selector screen
          await (window as any).ethereum.request({
            method: 'wallet_requestPermissions',
            params: [{ eth_accounts: {} }]
          });
        } catch (permErr) {
          console.warn('Wallet account select permission skipped or unsupported:', permErr);
        }
      }
  ...
  const switchNetwork = async (targetChainId: number) => {
    ...
    try {
      ...
      if (switchError.code === 4902) {
        try {
          const chainObj = targetChainId === 84538453 ? {
            chainId: hex,
            chainName: 'Base Vibenet',
            nativeCurrency: { name: 'Ether', symbol: 'ETH', decimals: 18 },
            rpcUrls: ['https://rpc.vibes.base.org'],
            blockExplorerUrls: ['https://explorer.vibes.base.org']
          } : targetChainId === 84532 ? {
            chainId: hex,
  ...
  const handleDeploy = async (e: React.FormEvent) => {
    ...
    // Fetch latest active account from provider to ensure we are in-sync
    ...
    try {
      ...
      console.warn('Failed to sync wallet address:', err);
    }
    ...
    try {
      ...
      if (B20_DEPLOYER_CONTRACT_ADDRESS && B20_DEPLOYER_CONTRACT_ADDRESS.startsWith('0x')) {
        ...
        const feeTxHash = await (window as any).ethereum.request({
          method: 'eth_sendTransaction',
          params: [{
            from: walletAddress,
        ...
        while (!feeReceipt) {
          ...
          feeReceipt = await (window as any).ethereum.request({
    ...
    try {
      ...
      try {
        ...
        for (let i = 0; i < calls.length; i++) {
          ...
          const singleHash = await (window as any).ethereum.request({
            method: 'eth_sendTransaction',
            params: [{
              from: activeAddress,
              to: calls[i].to,
  ...
  const fetchDeployerAdminDetails = async () => {
    ...
    try {
      ...
      // 1. Fetch Owner
      const ownerData = await provider.request({
        method: 'eth_call',
        params: [{
          to: B20_DEPLOYER_CONTRACT_ADDRESS,
          data: encodeFunctionData({ abi: B20_DEPLOYER_ABI, functionName: 'owner' })
        }, 'latest']
      });
      const decodedOwner = ownerData === '0x' || !ownerData ? '' : '0x' + ownerData.slice(26);
      setAdminOwner(decodedOwner);
      ...
      const feeData = await provider.request({
        ...
        params: [{
          to: B20_DEPLOYER_CONTRACT_ADDRESS,
          data: encodeFunctionData({ abi: B20_DEPLOYER_ABI, functionName: 'deployFee' })
        }, 'latest']
      });
      const decodedFee = feeData === '0x' || !feeData ? 0n : BigInt(feeData);
      setAdminDeployFee((Number(decodedFee) / 1e18).toString());

      // 3. Fetch Pending Owner
      const pendingData = await provider.request({
        method: 'eth_call',
        params: [{
          to: B20_DEPLOYER_CONTRACT_ADDRESS,
          data: encodeFunctionData({ abi: B20_DEPLOYER_ABI, functionName: 'pendingOwner' })
        }, 'latest']
      });
      const decodedPending = pendingData === '0x' || !pendingData ? '' : '0x' + pendingData.slice(26);
      setAdminPendingOwner(decodedPending === '0x0000000000000000000000000000000000000000' ? '' : decodedPending);

      // 4. Fetch Contract ETH Balance
      const balData = await provider.request({
        method: 'eth_getBalance',
        params: [B20_DEPLOYER_CONTRACT_ADDRESS, 'latest']
      });
      const decodedBal = balData === '0x' || !balData ? 0n : BigInt(balData);
      setAdminContractBalance((Number(decodedBal) / 1e18).toString());
        ...
        {walletModalOpen && (
          <div className="fixed inset-0 bg-[#07080a]/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <motion.div
              ...
              className="bg-[#0f1115] border border-white/5 rounded-3xl w-full max-w-sm p-6 shadow-2xl relative"
            >
              <div className="flex justify-between items-center mb-6">
                ...
                <button
                  ...
                  className="text-slate-400 hover:text-slate-100 font-bold text-sm cursor-pointer"
                >
                  ✕
                </button>
              </div>
              ...
              {!walletConnecting ? (
                <div className="space-y-2.5">
                  <button
                    ...
                    className="w-full bg-[#07080a] hover:bg-white/5 border border-white/5 p-4 rounded-2xl flex items-center justify-between text-xs font-bold text-slate-200 cursor-pointer transition-colors"
                  >
